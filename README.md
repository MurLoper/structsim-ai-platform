...
基于 React + TypeScript + Vite 的前端单页应用，用于配置管理、在线提单、订单列表、结果查看和权限控制。
...
- **现状与计划**: [CURRENT_STATUS_AND_PLAN.md](./docs/architecture/CURRENT_STATUS_AND_PLAN.md)
- **配置驱动指南**: [CONFIGURATION_GUIDE.md](./docs/development/CONFIGURATION_GUIDE.md)
...
- 后端 API 默认端口: `6060`
- Vite 会将 `/api` 代理到 `VITE_API_TARGET`，默认值为 `http://127.0.0.1:6060`
...
```
VITE_API_URL=http://127.0.0.1:6060/api/v1
VITE_API_TARGET=http://127.0.0.1:6060
```

说明：

- `VITE_API_URL` 用于前端请求基地址，默认值是 `/api/v1`
- `VITE_API_TARGET` 用于 Vite 开发代理目标地址
- 如果使用默认本地联调方式，只配置 `VITE_API_TARGET` 也可以
