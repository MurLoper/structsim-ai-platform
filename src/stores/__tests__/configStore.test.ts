...
describe('configStore', () => {
  ...
  it('fetchAllConfig 应填充基础配置数据', async () => {
    vi.mocked(configApi.getBaseData).mockResolvedValue({
      data: {
        simTypes: [{ id: 1, name: '结构', code: 'S', status: 'active' }],
        paramDefs: [{ id: 1, name: '参数A', code: 'P', status: 'active' }],
        conditionDefs: [{ id: 1, name: '工况', code: 'C', status: 'active' }],
        outputDefs: [{ id: 1, name: '结果', code: 'O', status: 'active' }],
        solvers: [{ id: 1, name: 'Solver', code: 'SOL', status: 'active' }],
        statusDefs: [{ id: 1, name: '完成', code: 'DONE' }],
        foldTypes: [{ id: 1, name: '姿态A', angle: 0 }],
        automationModules: [{ id: 1, name: 'Auto' }],
      },
    });

    vi.mocked(configApi.getProjects).mockResolvedValue({
      data: [{ id: 1, name: '项目A', code: 'P1', status: 'active' }],
    });
    vi.mocked(configApi.getWorkflows).mockResolvedValue({
      data: [{ id: 1, name: '流程A' }],
    });
    vi.mocked(configApi.getParamTplSets).mockResolvedValue({
      data: [{ id: 1, name: '模板A', simTypeId: 1 }],
    });
    vi.mocked(configApi.getCondOutSets).mockResolvedValue({
      data: [{ id: 1, name: '条件A', simTypeId: 1 }],
    });
  ...
  it('getStatus 与 getSimType 应返回匹配数据', () => {
    useConfigStore.setState({
      statusDefs: [{ id: 99, name: '完成', code: 'DONE' }],
      simTypes: [{ id: 77, name: '结构', code: 'S', status: 'active' }],
    });
  ...
  it('refreshProjects 应更新项目列表', async () => {
    vi.mocked(configApi.getProjects).mockResolvedValue({
      data: [{ id: 1, name: '项目B', code: 'P2', status: 'active' }],
    });
