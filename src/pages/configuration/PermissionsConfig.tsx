...
const TABS: TabItem[] = [
  ...
  { key: 'roles', label: '角色管理', icon: <ShieldCheck className="w-4 h-4" /> },
  { key: 'menus', label: '菜单管理', icon: <LayoutGrid className="w-4 h-4" /> },
];
...
const PermissionsConfig: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {activeTab === 'users' && (
        <Card>
          <CardHeader title="用户管理" icon={<Users className="w-5 h-5" />} />
      ...
      {activeTab === 'roles' && (
        <Card>
          <CardHeader title="角色管理" icon={<ShieldCheck className="w-5 h-5" />} />
      ...
      {activeTab === 'menus' && (
        <Card>
          <CardHeader title="菜单管理" icon={<LayoutGrid className="w-5 h-5" />} />
