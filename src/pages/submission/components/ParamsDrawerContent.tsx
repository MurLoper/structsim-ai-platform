...
import { DoeVerifyModal } from './DoeVerifyModal';
...
export const ParamsDrawerContent: React.FC<ParamsDrawerContentProps> = ({
  ...
  return (
    <div className="space-y-5">
            ...
            <div className="mt-3">
              <div className="flex items-center justify-between">
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 disabled:bg-slate-400 disabled:cursor-not-allowed rounded-lg transition-colors"
                >
              ...
              {/* DOE 验证错误提示 */}
              {doeValidationError && (
                <div className="mt-2 px-3 py-2 text-sm text-red-600 bg-red-50 dark:bg-red-900/20 dark:text-red-400 rounded-lg">
                  {doeValidationError}
                </div>
              )}
            </div>
      ...
      <DoeVerifyModal
        isOpen={showDoeModal}
        onClose={() => setShowDoeModal(false)}
        heads={config.params.optParams?.doeParamHeads || []}
        data={config.params.optParams?.doeParamData || []}
